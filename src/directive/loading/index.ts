...
// 创建加载遮罩层
function createLoadingMask(options: LoadingOptions = {}) {
  const {
    text = '加载中...',
...
// 显示加载状态
function showLoading(el: LoadingElement, binding: DirectiveBinding) {
...
// 隐藏加载状态
function hideLoading(el: LoadingElement) {
  if (!el.loadingInstance) return
